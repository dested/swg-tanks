...
const Horizontal: number = -3.4e38;
const Skip: number = -2;
...
function PointOnLineSegment(
  ...
  return (
    (fltEquals(pt.x, linePt1.x) && fltEquals(pt.y, linePt1.y)) ||
...
function InitEdge(e: TEdge, eNext: TEdge, ePrev: TEdge, pt: Point): void {
  e.next = eNext;
  e.prev = ePrev;
...
function IsMaxima(e: TEdge, y: number): boolean {
  return e != null && fltEquals(e.top.y, y) && e.nextInLML == null;
}

function IsIntermediate(e: TEdge, y: number): boolean {
  return fltEquals(e.top.y, y) && e.nextInLML != null;
}
...
function GetOverlap(
  ...
  b1: number,
  b2: number,
): {r: boolean; Left: number; Right: number} {
  let Left: number;
  let Right: number;
...
function PointInPolygonPath(pt: Point, path: Path): number {
  ...
  for (let i = 1; i <= cnt; ++i) {
    ...
    if (fltEquals(ipNext.y, pt.y)) {
      if (
        fltEquals(ipNext.x, pt.x) ||
        (fltEquals(ip.y, pt.y) && ipNext.x > pt.x == ip.x < pt.x)
      ) {
        return -1;
      }
    }
  ...
  private ProcessBound(E: TEdge, LeftBoundIsForward: boolean): TEdge {
    ...
    if (E.dx == Horizontal) {
      ...
      if (EStart.dx == Horizontal) {
        ...
        if (
          !fltEquals(EStart.bot.x, E.bot.x) &&
    ...
    if (LeftBoundIsForward) {
      while (
        fltEquals(Result.top.y, Result.next.bot.y) &&
      ...
      while (E != Result) {
        ...
        if (
          E.dx == Horizontal &&
          E != EStart &&
          !fltEquals(E.bot.x, E.prev.top.x)
        ) {
          ReverseHorizontal(E);
        }
        E = E.next;
      ...
      while (
        fltEquals(Result.top.y, Result.prev.bot.y) &&
      ...
      while (E != Result) {
        ...
        if (
          E.dx == Horizontal &&
          E != EStart &&
          !fltEquals(E.bot.x, E.next.top.x)
        ) {
          ReverseHorizontal(E);
        }
        E = E.prev;
...
export class Clipper extends ClipperBase {
  ...
  public Execute(
    ...
    fillType: PolyFillType = PolyFillType.pftEvenOdd,
  ): boolean {
  ...
  private SetWindingCount(edge: TEdge): void {
    ...
    if (e == null) {
      let pft: PolyFillType;
      pft =
        edge.polyTyp == PolyType.ptSubject
          ? this.m_SubjFillType
          : this.m_ClipFillType;
      if (edge.windDelta == 0) {
  ...
  private ProcessHorizontal(horzEdge: TEdge): void {
    ...
    for (;;) {
      ...
      while (e != null) {
        ...
        if (
          fltEquals(e.curr.x, horzEdge.top.x) &&
  ...
  private JoinPoints(j: Join, outRec1: OutRec, outRec2: OutRec): boolean {
    ...
    if (
      ...
      op1b = op1;
      while (
        fltEquals(op1.prev.pt.y, op1.pt.y) &&
        op1.prev != op1b &&
        op1.prev != op2
      ) {
        op1 = op1.prev;
      ...
      while (
        fltEquals(op1b.next.pt.y, op1b.pt.y) &&
        op1b.next != op1 &&
        op1b.next != op2
      ) {
        op1b = op1b.next;
      ...
      op2b = op2;
      while (
        fltEquals(op2.prev.pt.y, op2.pt.y) &&
        op2.prev != op2b &&
        op2.prev != op1b
      ) {
        op2 = op2.prev;
      ...
      while (
        fltEquals(op2b.next.pt.y, op2b.pt.y) &&
        op2b.next != op2 &&
        op2b.next != op1
      ) {
        op2b = op2b.next;
      ...
      let Reverse1 =
        op1b.pt.y > op1.pt.y || !SlopesEqual3P(op1.pt, op1b.pt, j.offPt);
      if (Reverse1) {
        op1b = op1.prev;
        while (op1b.pt.equals(op1.pt) && op1b != op1) {
          op1b = op1b.prev;
        }
        if (op1b.pt.y > op1.pt.y || !SlopesEqual3P(op1.pt, op1b.pt, j.offPt)) {
          return false;
        }
      }
      op2b = op2.next;
      ...
      let Reverse2 =
        op2b.pt.y > op2.pt.y || !SlopesEqual3P(op2.pt, op2b.pt, j.offPt);
      if (Reverse2) {
        op2b = op2.prev;
        while (op2b.pt.equals(op2.pt) && op2b != op2) {
          op2b = op2b.prev;
        }
        if (op2b.pt.y > op2.pt.y || !SlopesEqual3P(op2.pt, op2b.pt, j.offPt)) {
          return false;
        }
      }
  ...
  public static MinkowskiSumPath(
    pattern: Path,
    path: Path,
  ...
  public static MinkowskiSumPaths(
    pattern: Path,
    paths: Paths,
    pathIsClosed: boolean,
  ): Paths {
...
export class ClipperOffset {
  ...
  public constructor(
    ...
    arcTolerance: number = DefArcTolerance,
  ) {
  ...
  public AddPath(path: Path, joinType: JoinType, endType: EndType): void {
    ...
    if (this.m_lowest.x < 0) {
      ...
      let ip =
        this.m_polyNodes.children[this.m_lowest.x].polygon[this.m_lowest.y];
      if (
        newNode.polygon[k].y > ip.y ||
        (fltEquals(newNode.polygon[k].y, ip.y) && newNode.polygon[k].x < ip.x)
      ) {
        this.m_lowest = {x: this.m_polyNodes.childCount - 1, y: k};
      }
    }
  ...
  private FixOrientations(): void {
    ...
    if (
      ...
      for (let i = 0; i < this.m_polyNodes.childCount; i++) {
        let node = this.m_polyNodes.children[i];
        if (
          node.endType == EndType.etClosedLine &&
